<template>
  <div class="container">
    <div
      ...
      <form @submit.prevent="UpdateMatakuliah">
        <!-- JUDUL FORM -->
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.getMatakuliah(this.UpdateId);
    },
    methods: {
      getMatakuliah(UpdateId) {
        axios.get(BaseUrl + `/matakuliah/${UpdateId}`).then((res) => {
          this.matkul = res.data;
        });
      },
      UpdateMatakuliah() {
        axios
          .put(BaseUrl + `/matakuliah/${this.UpdateId}`, this.matkul)
          .then((res) => {
