  ...
  <div>
    <div class="container mx-auto">
      <div
        ...
        <form @submit.prevent="SimpanDataDosen">
          <!-- JUDUL FORM -->
          ...
          <!-- INPUT NAME KATEGORI KEGIATAN -->
          <div class="relative z-0 w-full mb-5 group">
            <input type="hidden" v-model="matakuliahId" />
            <input
              type="text"
              name="floating_password"
              v-model="data_dosen.nama_dosen"
          ...
          <!-- END-INPUT SKS MATAKULIAH -->
          <!-- INPUT SKS MATAKULIAH -->
          <div class="relative z-0 w-full mb-5 group">
            <input
              type="text"
              name="floating_password"
              v-model="data_dosen.prodi"
              id="floating_password"
              class="block py-1.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
              placeholder=""
              required />
          ...
          <div class="mt-4 flex items-center justify-end gap-x-6">
            ...
            <button
              type="submit"
              class="rounded-md bg-blue-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-400 flex items-center p-2 space-x-3">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.matakuliahId = this.$route.params.adddosen;
      this.getIdMatKul(this.matakuliahId);
    },
    methods: {
      //function untuk menghandle id matakuliah
      getIdMatKul(matakuliahId) {
        axios.get(BaseUrl + `/matakuliah/${matakuliahId}`).then((res) => {
          this.data_dosen = res.data; //ambil id matakuliah yang di kirim melalui url
        });
      },
      SimpanDataDosen() {
        axios
          .post(
            BaseUrl + `/matakuliah/${this.matakuliahId}/dosen`,
            this.data_dosen
          )
          .then((res) => {
            alert("data berhasil di tambahkan");
            this.data_dosen.nama_dosen = "";
            this.data_dosen.gelar = "";
            this.data_dosen.prodi = "";
          });
      },
    },
  };
